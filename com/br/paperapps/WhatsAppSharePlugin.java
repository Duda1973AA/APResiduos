...
public class WhatsAppSharePlugin extends Plugin {
    ...
    public void load() {
        this.broadcastReceiver = new BroadcastReceiver() { // from class: com.br.paperapps.WhatsAppSharePlugin.1
            @Override // android.content.BroadcastReceiver
            public void onReceive(Context context, Intent intent) {
                if (Build.VERSION.SDK_INT >= 33) {
                    WhatsAppSharePlugin.this.chosenComponent = (ComponentName) intent.getParcelableExtra("android.intent.extra.CHOSEN_COMPONENT", ComponentName.class);
                } else {
                    WhatsAppSharePlugin whatsAppSharePlugin = WhatsAppSharePlugin.this;
                    whatsAppSharePlugin.chosenComponent = whatsAppSharePlugin.getParcelableExtraLegacy(intent, "android.intent.extra.CHOSEN_COMPONENT");
                }
            }
        };
        if (Build.VERSION.SDK_INT >= 33) {
            getActivity().registerReceiver(this.broadcastReceiver, new IntentFilter("android.intent.extra.CHOSEN_COMPONENT"), 2);
        } else {
    ...
    private void shareFiles(JSArray jSArray, Intent intent, PluginCall pluginCall) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (isFileUrl(str)) {
                    getMimeType(str);
                    Uri uriForFile = FileProvider.getUriForFile(getActivity(), getContext().getPackageName() + ".fileprovider", new File(Uri.parse(str).getPath()));
                    arrayList.add(uriForFile);
                    Log.e("WhatsAppShare imageUri", uriForFile.toString());
                } else {
